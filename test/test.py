            ...
            for index, line in enumerate(f.readlines()):
                ...
                if "EXTDEF" in instruction_arr:
                    if instruction_arr.index("EXTDEF") != 0:
                        raise SyntaxError(
                            f'line {index + 1}: "EXTDEF can not have symbol"'
                        )
                ...
                # special process EXTREF
                    ...
                    if instruction_arr.index("EXTREF") != 0:
                        raise SyntaxError(
                            f'line {index + 1}: "EXTREF can not have symbol"'
                        )
        ...
        for index, instr in enumerate(self.instruction):
            ...
            # clear literal
                ...
                for literal in self.__literal_table:
                    ...
                    self.instruction.insert(
                        it,
                        {"symbol": "*", "mnemonic": literal, "location": cur_location},
                    )
            ...
            if instr["mnemonic"] == "EQU":
                ...
                if instr["operand"] == "*":
                    ...
                    cur_symbol_table[instr["symbol"]] = (
                        cur_symbol_table[symbol_1] - cur_symbol_table[symbol_2]
                    )
                elif "+" in instr["operand"]:
                    symbol_1, symbol_2 = instr["operand"].split("+")
                    cur_symbol_table[instr["symbol"]] = (
        ...
        for index, instr in enumerate(self.instruction):
            ...
            # literal instruction
                ...
                if "-" in instr["operand"]:
                    symbol_1, symbol_2 = instr["operand"].split("-")
                    location_1 = (
                        self.__symbol_table[cur_block][symbol_1]
                        if symbol_1 in self.__symbol_table[cur_block]
                        else 0
                    )
                    if location_1 == 0:
                    ...
                    location_2 = (
                        self.__symbol_table[cur_block][symbol_2]
                        if symbol_2 in self.__symbol_table[cur_block]
                        else 0
                    )
                    if location_2 == 0:
                        cur_modified_list.append(
                ...
                if "opcode" in instr:
                    ...
                    if instr["operand"][0] == "#":
                        ...
                        if token in self.__symbol_table[cur_block]:
                            ...
                            if offset >= -2048 and offset <= 2047:
                                instr["opcode"] = self.__gen_code_list(
                                    instr["mnemonic"], 1, 2, offset
                                )
                            else:
                                ...
                                if offset >= 0 and offset <= 4095:
                                    instr["opcode"] = self.__gen_code_list(
                                        instr["mnemonic"], 1, 4, offset
                                    )
                                # format 4
                                else:
                                    instr["opcode"] = self.__gen_code_list(
                                        instr["mnemonic"][1:], 1, 1, symbol_loc
                                    )
                        # operand is number
                        else:
                            ...
                            if offset > 4095:
                                instr["opcode"] = self.__gen_code_list(
                                    instr["mnemonic"][1:], 1, 1, offset
                                )
                            # format 3
                            else:
                                instr["opcode"] = self.__gen_code_list(
                                    instr["mnemonic"], 1, 0, offset
                                )
                    # indirect format (n: 1, i: 0)
                    elif instr["operand"][0] == "@":
                        symbol = instr["operand"][1:]
                        symbol_loc = (
                            self.__symbol_table[cur_block][symbol]
                            if symbol in self.__symbol_table[cur_block]
                            else None
                        )
                        # symbol not defined
                        if symbol_loc == None:
                            raise SyntaxError(
                                f"line {index + 1}: symbol has not been defined"
                            )
                        else:
                            # calculate offset
                            offset = symbol_loc - instr["location"] - 3
                            ...
                            if offset >= -2048 and offset <= 2047:
                                ...
                                if offset >= 0 and offset <= 4095:
                                    instr["opcode"] = self.__gen_code_list(
                                        instr["mnemonic"], 2, 4, offset
                                    )
                                # format 4
                                else:
                                    instr["opcode"] = self.__gen_code_list(
                                        instr["mnemonic"], 2, 1, symbol_loc
                                    )
                    # direct format (n: 1, i: 1)
                        ...
                        # x label
                        if (
                            isinstance(instr["operand"], list)
                            and "X" in instr["operand"]
                        ):
                            format_num |= 8
                        # format 4
                        if instr["mnemonic"][0] == "+":
                            ...
                            # get symbol location
                            try:
                                symbol_loc = self.__symbol_table[cur_block][
                                    first_element
                                ]
                            except:
                                symbol_loc = None
                            ...
                            if symbol_loc == None:
                                ...
                                instr["opcode"] = self.__gen_code_list(
                                    mnemonic, 3, format_num, symbol_loc
                                )
                        else:
                            symbol_loc = None
                            # get first element
                            ...
                            if symbol_loc == None:
                                ...
                                if offset >= -2048 and offset <= 2047:
                                    format_num |= 2
                                    instr["opcode"] = self.__gen_code_list(
                                        instr["mnemonic"], 3, format_num, offset
                                    )
                                # format 3 (B)
                                else:
                                    format_num |= 4
            ...
            if instr["mnemonic"] == "START":
                ...
                for block in self.__symbol_table.keys():
                    if instr["operand"] in self.__symbol_table[block]:
                        end_position = (
                            block,
                            self.__symbol_table[block][instr["operand"]],
                        )
